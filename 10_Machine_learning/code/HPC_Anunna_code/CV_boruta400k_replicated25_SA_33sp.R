...
ps_sp_l$Bac
load(file = "./10_Machine_learning/r_objects/Boruta_SA_bac_33sp_400k.RData") #CHANGEd to 400k
...
t0<-Sys.time()
set.seed(23456)
replicated_model_400k_bac_SA<-replicate(25, 
                                 fix_split_train_test_replicated(boruta_output_l = Boruta_stress_bac_33sp[-24],
                                                                 ps_object_l = lapply(ps_sp_l$Bac[-24], function (x)
                                                                   subset_samples(x,
                                                                                  Stress %in% c("Control",
                                                                                                "SA"))),
...
# now train fungi
# note: specie 31, maize, has zero predictors and would crash the formula

t0<-Sys.time()
set.seed(23456)
replicated_model_400k_fun_SA<-replicate(25, 
                                 fix_split_train_test_replicated(boruta_output_l = Boruta_stress_fun_33sp[-31],
...
CV_25replicated_SA_400k_l_l_33sp<-list("replicated_model_400k_bac_SA" = replicated_model_400k_bac_SA,
                                         "replicated_model_400k_fun_SA" = replicated_model_400k_fun_SA)

save(CV_25replicated_SA_400k_l_l_33sp, file = "./10_Machine_learning/r_objects/CV_25replicated_SA_400k_l_l_33sp.RData")
